import { events, expectedChronology } from '../data.js';
...
if (expectedChronology.length !== events.length) throw new Error(`Chronology list has ${expectedChronology.length} ids for ${events.length} events`);
...
for (const [i, event] of events.entries()) {
  ...
  if (event.id !== expectedChronology[i]) throw new Error(`Event ${i+1} should be ${expectedChronology[i]}, got ${event.id}`);
